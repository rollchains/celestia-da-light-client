...
func (misbehaviour Misbehaviour) ValidateBasic() error {
	...
	if misbehaviour.Header1.Header.ChainID != misbehaviour.Header2.Header.ChainID {
		return fmt.Errorf("invalid mishbehaviour, headers must have identical chainIDs")
	}

	if err := ClientIdentifierValidator(misbehaviour.ClientId); err != nil {
		return errorsmod.Wrap(err, "misbehaviour client ID is invalid")
	}

	// ValidateBasic on both validators
	if err := misbehaviour.Header1.ValidateBasic(); err != nil {
		return fmt.Errorf("invalid misbehaviour, header 1 failed validation, %v", err)
	}
	if err := misbehaviour.Header2.ValidateBasic(); err != nil {
		return fmt.Errorf("invalid misbehaviour, header 2 failed validation, %v", err)
	}
	...
	if misbehaviour.Header1.GetHeight().LT(misbehaviour.Header2.GetHeight()) {
		return fmt.Errorf("invalid misbehaviour, header 1 height is less than header 2 height (%s < %s)", misbehaviour.Header1.GetHeight(), misbehaviour.Header2.GetHeight())
	}
...
func validCommit(chainID string, blockID cmttypes.BlockID, commit *cmtproto.Commit, valSet *cmtproto.ValidatorSet) (err error) {
	...
	if err := tmValset.VerifyCommitLight(chainID, blockID, tmCommit.Height, tmCommit); err != nil {
		return fmt.Errorf("invalid misbehaviour, validator set did not commit to header")
	}
